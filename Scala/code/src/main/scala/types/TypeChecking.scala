...
object TestTypeChecking extends App {
  ...
  def matchGenericWithTypeTag[A: TypeTag](list: List[A]) =
    list match {
      case strlist: List[A] if typeOf[A] =:= typeOf[String] =>
        println("A list of strings! " + strlist.head)
      case intlist: List[A] if typeOf[A] =:= typeOf[Int] =>
        println("A list of ints! " + intlist.head)
    }
  ...
  println(
    this.getClass.getDeclaredMethods.toList
      .filter(_.getName() == "matchGenericWithTypeTag")
      .head
  )

  val c = this ## //HashCode
  val d =
    this -> "a" // Wrap this and other in to a tuple, like an entry of the map
  /**
