...
/**
 * Monadic Try type. Represents a result type that could have succeeded with
 * type T or failed with a Throwable. Originally was Exception but due to seeing
 * issues with eg play with checked Throwable, And also seeing that Scala deals
 * with throwable, I made the decision to change it to use Throwable.
 *
 * @param <T>
 */
...
public abstract class Try<T> {
	...
	/**
	 * Transform success or pass on failure. Takes an optional type parameter of
	 * the new type. You need to be specific about the new type if changing type
	 * <p>
	 ...
	 * Takes an optional type parameter of the new type. You need to be specific
	 * about the new type if changing type.
	 * <p>
	 * Try.ofFailable(() -> "1").<Integer>flatMap((x) -> Try.ofFailable(()
	 * ->Integer.valueOf(x))) returns Integer(1)
	 *
	 * @param f
	 *            function to apply to successful value.
	 * @param <U>
	...
	/**
	 * Specifies a result to use in case of failure. Gives access to the
	 * exception which can be pattern matched on.
	 * <p>
	 * Try.ofFailable(() -> "not a number") .<Integer>flatMap((x) ->
	 * Try.ofFailable(() ->Integer.valueOf(x))) .recover((t) -> 1) returns
	 * Integer(1)
	 *
	 * @param f
	 * @return
	...
	public abstract Try<T> recoverWith(
			TryMapFunction<? super Throwable, Try<T>> f);

	/**
	 * Return a value in the case of a failure. This is similar to recover but
	 * does not expose the exception type.
	 *
	 * @param value
	 * @return
	 */
	...
	/**
	 * Return another try in the case of failure. Like recoverWith but without
